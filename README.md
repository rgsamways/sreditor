...
Sreditor correlates an archived change to a real diff by finding the git commit that added its `openspec/changes/archive/<id>/` folder, then widens the diff backward to the commit that first added the change's *draft* `proposal.md` — so the full implementation window is captured, not just whatever the final archiving commit happens to contain. This matters because an `archive` commit is often just a folder move with no code in it; without the wider window, workflows with many commits between implementing and archiving a change would see nothing but that move. **Known limitation:** if no draft commit can be found (e.g. history predating this convention, or a rewritten history where the draft commit isn't an ancestor of the archiving commit), it falls back to the archiving commit alone. `scc`/`jscpd` also run against the *current* working-tree state of the changed files, not their state at the time of the historical commits.
